...
def align_data(embedding_mat, cue_to_idx, norm_df, norm_name, verbose: bool = False):
    ...
    dropped_swow = embedding_vocab_size - overlap_size
    dropped_norms = norm_vocab_size - overlap_size
    
    if verbose:
        print(f"    > Overlap: {overlap_size}/{norm_vocab_size} ({overlap_pct:.1%})")
        print(f"    > Drop Stats: {dropped_swow} SWOW cues unused, {dropped_norms} Norm words missing.")
    
        if len(overlap) > 0:
            print(f"    > Sample Overlap: {overlap[:5]}")
            missing = list(available_words - valid_cues)
            if missing:
                print(f"    > Sample Missing (in Norms but not SWOW): {missing[:5]}")
...
def run_evaluation_loop(embeddings_dict, mappings, norms_df, output_path, allowed_models=None, verbose: bool = False, n_jobs: int = 1):
...
def main():
    ...
    run_evaluation_loop(embeddings, mappings, norms_df, output_csv, allowed_models=args.models, verbose=args.verbose, n_jobs=args.n_jobs)
