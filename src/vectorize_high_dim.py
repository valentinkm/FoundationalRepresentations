...
#   - 'model_X_passive': Dense Matrix (SVD of PPMI, matched to Activation Dims)
#   - 'model_Y_active': Dense Matrix (SVD of PPMI, matched to Activation Dims)
"""
...
def _process_single_transform(key: str, mat: csr_matrix, verbose: bool, activation_dims: dict):
    """Helper for parallel PPMI+SVD with variable Dimensions."""
    if key == 'mappings': return None
    
    # Infer model name to find target dimension
    # Key is 'passive_modelname' or 'active_modelname'
    clean_name = key.replace('passive_', '').replace('active_', '')
    
    if clean_name not in activation_dims:
        if verbose:
            print(f"  [Skip] {key}: No matching activation dimension found.")
        return None
        
    target_dim = activation_dims[clean_name]
    ...
    if k < 2:
        ...
        try:
            ...
            # If rank deficient for target dim, pad
...
def derive_dense_embeddings(matrices: dict, activation_dims: dict, verbose: bool = False, n_jobs: int = 1) -> dict:
    """
    Convert Sparse Count/Prob Matrices -> PPMI -> SVD Dense Vectors (High Dim).
    """
    print(f"[Transformation] Applying PPMI + SVD (High Dim) with n_jobs={n_jobs}...")
    
    results = Parallel(n_jobs=n_jobs)(
        delayed(_process_single_transform)(key, mat, verbose, activation_dims)
...
def main():
    ...
    parser.add_argument('--activation_dir', type=Path, required=True, help="Dir containing Activation CSVs (REQUIRED for Target Dims)")
    ...
    if args.activation_dir:
        ...
        print("[Error] Activation Dir required for High Dim Vectorization.")
        return
    ...
    # 4. Transform (PPMI -> SVD High Dim)
    # Note: We do NOT pass n_components argument anymore
    dense_results = derive_dense_embeddings(matrices, activation_dims=activation_dims, verbose=args.verbose, n_jobs=args.n_jobs)
    
    # 5. Export
    # Merge dense results (High Dim) with raw activation matrices
    ...
    # Save as embeddings_high_dim.pkl
    out_path = args.output_dir / "embeddings_high_dim.pkl"
